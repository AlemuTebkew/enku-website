...
// Define the props for the FilterDrawer component
...
// Define the FilterDrawer component
const FilterDrawer: React.FC<CartDrawerProps> = ({
  isOpen,
  onClose,
  filters,
  selectedFilters,
  ...
  // State for managing the local filters
  const [localSelectedFilters, setLocalSelectedFilters] = useState<number[]>(selectedFilters);
  // State for managing the selected filter accordion
  const [selectedFilter, setSelectedFilter] = useState<number>(0);

  // Initialize local filters when the component mounts or when selectedFilters change
  useEffect(() => {
      console.log("useEffect in FilterDrawer: Initializing local filters with props", selectedFilters);
  ...
  const handleFilterChange = useCallback((filterId: number) => {
    console.log("handleFilterChange: filterId = ", filterId);
    // Update the internal list of selected filters
    setLocalSelectedFilters(prevFilters => {
      const updatedFilters = prevFilters.includes(filterId)
        ? prevFilters.filter(id => id !== filterId)
        : [...prevFilters, filterId];

      console.log("handleFilterChange: updatedFilters", updatedFilters);
    ...
    >
      <div className="h-full flex flex-col relative">
        ...
        <div className="flex justify-between items-center p-4 bg-[#FFD1DF] fixed top-0 w-full z-50">
          ...
          <div onClick={() => setLocalSelectedFilters([])} className="flex items-center cursor-pointer mr-2 gap-2">
    ...
    <p className="text-lg">No filters available</p>
         ...
         {/* Filter values section */}
          <div className="w-full h-full flex flex-col gap-4 py-4 bg-background px-4 rounded-t-lg overflow-auto">
            {filters &&
              filters.length > 0 &&
              filters[selectedFilter].values.map((value, index) => {
                const isChecked = localSelectedFilters.includes(value.id);
                console.log(`Checkbox for ${value.value} isChecked: ${isChecked}`);
                return (
                  <div className="flex flex-col gap-2" key={`${value.value}-${value.id}`}>
        ...
        <div className="w-full flex justify-between items-center p-4 bg-background fixed bottom-0 z-50">
          <Button
            onClick={() => {
              onSetFilter(localSelectedFilters);
              onClose(false);
            }}
